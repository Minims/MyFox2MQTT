...
class MyFoxWebsocket:
    """MyFox WebSocket Class"""
    ...
    def update_keyfob_presence(self, message):
        ...
        topic = f"{self.mqtt_config.get('topic_prefix', 'myFox2mqtt')}/{site_id}/{device_id}/presence"
    ...
    def security_level_change(self, message):
        ...
        topic = f"{self.mqtt_config.get('topic_prefix', 'myFox2mqtt')}/{site_id}/state"
    ...
    def alarm_trespass(self, message):
        ...
        if device_type == "pir":
            ...
            topic = f"{self.mqtt_config.get('topic_prefix', 'myFox2mqtt')}/{site_id}/{device_id}/pir"
    ...
    def default_message(self, message):
        ...
        device_id = message.get("device_id")
        if not site_id:
            topic = f"{self.mqtt_config.get('topic_prefix', 'myFox2mqtt')}/{topic_suffix}"
        elif not device_id:
            topic = f"{self.mqtt_config.get('topic_prefix', 'myFox2mqtt')}/{site_id}/{topic_suffix}"
        else:
            topic = f"{self.mqtt_config.get('topic_prefix', 'myFox2mqtt')}/{site_id}/{device_id}/{topic_suffix}"
        mqtt_publish(
            mqtt_client=self.mqtt_client,
            topic=topic,
            payload=message,
